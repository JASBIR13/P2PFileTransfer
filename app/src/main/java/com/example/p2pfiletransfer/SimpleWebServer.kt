...
class SimpleWebServer(port: Int, val uploadDir: File, private val onClipboardUpdate: (String) -> Unit) : NanoHTTPD(port) {
    ...
    private fun serveFileList(): Response {
        ...
        uploadDir.listFiles()?.forEach { file ->
            ...
            html.append("<li><a href=\"/files/$encodedFileName\">${escapeHtml(file.name)}</a> - <form action=\"/delete/$encodedFileName\" method=\"post\"><button type=\"submit\">Delete</button></form></li>")
        }
    ...
    private fun handleFileUpload(session: IHTTPSession): Response {
        return try {
            ...
            newFixedLengthResponse(Response.Status.OK, "text/plain", "File uploaded successfully: $fileName")
        } catch (e: Exception) {
            Log.e(TAG, "Error handling file upload", e)
            newFixedLengthResponse(Response.Status.INTERNAL_ERROR, "text/plain", "An error occurred during file upload")
        }
    }
     ...
     */
    private fun deleteFile(fileName: String): Response {
        return try {
            if (!isValidFileName(fileName)) {
                return newFixedLengthResponse(Response.Status.FORBIDDEN, "text/plain", "Invalid file name")
            }
            ...
            if (file.exists()) {
                ...
                if (deleted) {
                    ...
                    newFixedLengthResponse(Response.Status.OK, "text/plain", "File deleted successfully: $fileName")
                } else {
